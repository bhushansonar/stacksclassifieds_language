<div class="container top">
    ...
    if ($this->session->flashdata('flash_message')) {
        if ($this->session->flashdata('flash_message') == 'add') {
            ...
            echo '</div>';
            $this->session->set_userdata('flash_message', '');
        } else if ($this->session->flashdata('flash_message') == 'price') {
            echo '<div class="alert alert-success">';
            echo '<a class="close" data-dismiss="alert">&#215;</a>';
            echo '<strong>Well done!</strong> price updated with success for all categories.';
            echo '</div>';
            $this->session->set_userdata('flash_message', '');
        } else {
...
//save the columns names in a array that we will use as filter
                ...
                <select name="order" class="span2">
                    <option <?php echo ($order == 'category_name') ? 'selected="selected"' : "" ?> value="category_name_en">Category Name</option>
...
//echo form_dropdown('order', $options_category, $order, 'class="span2"');
            ...
            <table class="table table-striped table-bordered table-condensed">
                <thead>
                    <tr>
<!--                        <th class="header">#</th>-->
                        <th class="yellow header headerSortDown">Category Name en</th>
