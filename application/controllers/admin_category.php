...
class Admin_category extends CI_Controller {
    ...
    public function add() {
        ...
        if ($this->input->server('REQUEST_METHOD') === 'POST') {
            ...
            $this->form_validation->set_rules('category_name_en', 'Category Name English', 'required');
            ...
            if ($this->form_validation->run()) {
                if ($this->input->post('parent_id') != 0) {
                    $path1 = addslashes($this->category_model->getCategoryPathById($this->input->post('parent_id'))) . " > ";
                ...
                for ($i = 0; $i < count($site_language); $i++) {
                    $insertArr['category_name_' . $site_language[$i]['language_shortcode']] = $this->input->post('category_name_' . $site_language[$i]['language_shortcode']);
                }
    ...
    public function update() {
        ...
        if ($this->input->server('REQUEST_METHOD') === 'POST') {
            ...
            if (isset($_POST['is_adult']) && !empty($_POST['is_adult'])) {
...
//if the form has passed through the validation
            if ($this->form_validation->run()) {
                ...
                $insertArr['parent_id'] = $this->input->post('parent_id');
                $insertArr['category_name_en'] = $this->input->post('category_name_en');
                $insertArr['category_name_fr'] = $this->input->post('category_name_fr');
                $insertArr['category_name_de'] = $this->input->post('category_name_de');
                $insertArr['path'] = $path;
                $insertArr['price'] = $this->input->post('price');
                $insertArr['display_order'] = $this->input->post('display_order');
                $insertArr['is_adult'] = $is_adult;

                if ($this->category_model->update_category($id, $insertArr) == TRUE) {
