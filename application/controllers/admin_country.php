...
class Admin_country extends CI_Controller {
    ...
    public function __construct() {
        ...
        if (!Access_level::get_access('country')) {
    ...
    public function add() {
        ...
        if ($this->input->server('REQUEST_METHOD') === 'POST') {
            ...
            $this->form_validation->set_rules('currency_type', 'Currency Type', 'required');
            $this->form_validation->set_error_delimiters('<div class="alert alert-error"><a class="close" data-dismiss="alert">&#215;</a><strong>', '</strong></div>');
            //if the form has passed through the validation
            if ($this->form_validation->run()) {
                $insertArr = array();
                $insertArr['country_name'] = $this->input->post('country_name');
                $insertArr['language_shortcode'] = $this->input->post('country_name');
                $insertArr['currency_type'] = $this->input->post('currency_type');
                //if the insert has returned true then we show the flash message
                if ($this->country_model->store_country($insertArr)) {
    ...
    public function update() {
        ...
        //if save button was clicked, get the data sent via post
        if ($this->input->server('REQUEST_METHOD') === 'POST') {
            $this->form_validation->set_rules('country_name', 'Country Name', 'required|edit_unique[country.country_name.' . $id . ']');
            $this->form_validation->set_rules('currency_type', 'Currency Type', 'required');
            $this->form_validation->set_error_delimiters('<div class="alert alert-error"><a class="close" data-dismiss="alert">&#215;</a><strong>', '</strong></div>');
            if ($this->form_validation->run()) {
